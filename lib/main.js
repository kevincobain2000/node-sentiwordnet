...
var Senti = function(synset, redisDB) {
    this.synset = synset;
...
Senti.prototype.get = function(callback) {
    var self = this;
    var redisEntryCheck = self.redisEntryCheck(function(reply) {
        callback(reply);
    });
}; // end get()
...
var SentiObj = new Senti("basipetal#1", 2);
//console.log(x.get());
SentiObj.get(function(reply) {
    console.log(reply);
});
